...
VERSION=
commit=false
url=https://github.com/ankerproject/anker
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the anker, gitian-builder, gitian.sigs, and anker-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/ankerproject/anker
...
then
	# Make output folder
	mkdir -p "./anker-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit anker=${COMMIT} --url anker=${url} ../anker/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../anker/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/anker-*.tar.gz build/out/src/anker-*.tar.gz ../anker-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit anker=${COMMIT} --url anker=${url} ../anker/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/anker-*.zip build/out/anker-*.exe ../anker-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit anker=${COMMIT} --url anker=${url} ../anker/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/anker-*.tar.gz build/out/anker-*.dmg ../anker-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit anker=${COMMIT} --url anker=${url} ../anker/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../anker/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/anker-*.tar.gz build/out/src/anker-*.tar.gz ../anker-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../anker/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/anker-*win64-setup.exe ../anker-binaries/${VERSION}
	    mv build/out/anker-*win32-setup.exe ../anker-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../anker/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/anker-osx-signed.dmg ../anker-binaries/${VERSION}/anker-${VERSION}-osx.dmg
	fi
	popd || exit
