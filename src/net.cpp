...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Anker users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Anker " + FormatFullVersion();
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        if (addrman.size() < 2 && (GetTime() - nStart > 60)) {
...
void ThreadOpenAddedConnections()
{
    {
        LOCK(cs_vAddedNodes);
        mapMultiArgs["-addnode"].push_back("178.128.167.26:12365");
        mapMultiArgs["-addnode"].push_back("178.128.167.244:12365");
        mapMultiArgs["-addnode"].push_back("178.128.165.28:12365");
        mapMultiArgs["-addnode"].push_back("178.128.173.107:12365");
        mapMultiArgs["-addnode"].push_back("178.128.163.187:12365");
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Anker Core is probably already running."), addrBind.ToString());
        else
