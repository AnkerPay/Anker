...
#ifndef ANKER_ZANKWALLET_H
#define ANKER_ZANKWALLET_H
...
public:
    CzANKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZANK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZANK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ANKER_ZANKWALLET_H
