...
CzANKWallet::CzANKWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        if (walletdb.ReadZANKSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZANKSeed_deprecated()) {
...
bool CzANKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZANKCount(nCountLastUsed);
    else if (!walletdb.ReadZANKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzANKWallet::Lock()
{
    seedMaster = 0;
}

void CzANKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzANKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZANK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzANKWallet::LoadMintPoolFromDB()
{
...
void CzANKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzANKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzANKWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzANKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZANK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzANKWallet::SeedToZANK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzANKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzANKWallet::GenerateDeterministicZANK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzANKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzANKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
