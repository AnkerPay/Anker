...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake) {
        ...
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock);
        CMutableTransaction txEmptyCoinStake;
        pblock->vtx[0].vout[0].SetEmpty();
        pblock->vtx.push_back(CTransaction(txEmptyCoinStake)); // updated at end
    }
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zANK spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zAnk spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zANK serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        LogPrintf("CreateNewBlock(): total size %u txs: %u fees: %s sigopscost %d\n", nBlockCost, nBlockTx, FormatMoney(nFees).c_str(), nBlockSigOpsCost);
        ...
        if (fZerocoinActive && nHeight > 10) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zANK will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AnkerMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AnkerMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AnkerMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("anker-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AnkerMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
