...
#include "ui_zankcontroldialog.h"
...
std::set<std::string> ZAnkControlDialog::setSelectedMints;
std::set<CMintMeta> ZAnkControlDialog::setMints;

ZAnkControlDialog::ZAnkControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAnkControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZAnkControlDialog::~ZAnkControlDialog()
{
    delete ui;
}

void ZAnkControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAnkControlDialog::updateList()
{
...
void ZAnkControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAnkControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAnk_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAnkControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAnkControlDialog::GetSelectedMints()
{
...
void ZAnkControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
