...
extern std::vector<std::string> invoices;
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    overviewAction->setObjectName("overviewAction"); // Name for CSS addressing
#ifdef Q_OS_MAC
    ...
    sendCoinsAction->setObjectName("sendCoinsAction"); // Name for CSS addressing
#ifdef Q_OS_MAC
    ...
    receiveCoinsAction->setObjectName("receiveCoinsAction"); // Name for CSS addressing
#ifdef Q_OS_MAC
    ...
    historyAction->setObjectName("historyAction"); // Name for CSS addressing
#ifdef Q_OS_MAC
    ...
    privacyAction->setObjectName("privacyAction"); // Name for CSS addressing
#ifdef Q_OS_MAC
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Anker Core"), this);
    aboutAction->setStatusTip(tr("Show information about Anker Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Anker"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Anker addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Anker addresses"));
    ...
    openAction->setStatusTip(tr("Open a Anker: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Anker Core help message to get a list with possible Anker command-line options"));
...
void BitcoinGUI::copyAnkerWallet()
{
    GUIUtil::setClipboard(QString::fromStdString(EncodeDestination(pwalletMain->vchDefaultKey.GetID())));
}

void BitcoinGUI::toggleDock()
{
    if (dock->isHidden()) {
        dock->show();
        kyc_name_edit->setText(QString::fromStdString(kycinfo.fio));
        kyc_email_edit->setText(QString::fromStdString(kycinfo.email));
...
void BitcoinGUI::toggleStyle()
{
    ...
    if (!settings.contains("fLightTheme")) {
        settings.setValue("fLightTheme", true);
        fLightTheme = true;
    } else if (settings.value("fLightTheme").toBool()) {
    ...
    if (fLightTheme) {
        QFile qFile(QString(":/css/light"));
        if (qFile.open(QFile::ReadOnly)) {
            styleSheet = QLatin1String(qFile.readAll());
        }
        this->setStyleSheet(styleSheet);
    } else {
        QFile qFile(QString(":/css/default"));
        if (qFile.open(QFile::ReadOnly)) {
            styleSheet = QLatin1String(qFile.readAll());
        }
        this->setStyleSheet(styleSheet);
...
void BitcoinGUI::createToolBars()
{
    if (walletFrame) {
        ...
        spacer1_1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        auto spacer2_1 = new QWidget(this);
        ...
        */
        ...
        QHBoxLayout* homelogolayoutH = new QHBoxLayout;
        QPixmap fulllogotype(":/images/logo_transparent");
        QLabel* fulllogotypeLabel = new QLabel;
        fulllogotypeLabel->setPixmap(fulllogotype);
        auto spacerlogo1 = new QWidget(this);
        spacerlogo1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        auto spacerlogo2 = new QWidget(this);
        spacerlogo2->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        homelogolayoutH->addWidget(spacerlogo1);
        homelogolayoutH->addWidget(fulllogotypeLabel);
        homelogolayoutH->addWidget(spacerlogo2);
        ...
        spacerhome1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        auto spacerhome2 = new QWidget(this);
        spacerhome2->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        ...
        if (walletFrame) {
            ...
            settingsmenu->addAction(unlockWalletAction);
            settingsmenu->addAction(lockWalletAction);
            settingsmenu->addAction(bip38ToolAction);
            settingsmenu->addAction(multiSendAction);
        ...
        sbarwidget->setObjectName("StatusBar"); // Name for CSS addressing
        sbarwidget->setFixedHeight(26);
        QHBoxLayout* statusbarwidget = new QHBoxLayout(sbarwidget);
        statusbarwidget->setSpacing(0);
    ...
    //    progressBar->setVisible(true);
        ...
        statusbarwidget->addWidget(progressBarLabel);
        statusbarwidget->addWidget(progressBar);
        statusbarwidget->addWidget(statusbarspacer);
        ...
        QWidget *personalKYC;
        personalKYC = new QWidget(dock);
        personalKYC->setObjectName("personalKYC"); // Name for CSS addressing
        personalKYC->setWindowFlags(personalKYC->windowFlags() | Qt::FramelessWindowHint);
        QVBoxLayout* personalKYClayout = new QVBoxLayout;
        //kycinfo.Readdb();
        personalKYClayout->addStretch();
        QLabel* kyc_name_label = new QLabel();
        kyc_name_label->setText(QString("Name"));
        kyc_name_edit = new QLineEdit;
        kyc_name_edit->setText(QString::fromStdString(kycinfo.fio));
        personalKYClayout->addWidget(kyc_name_label);
        personalKYClayout->addWidget(kyc_name_edit);
        QLabel* kyc_email_label = new QLabel();
        kyc_email_label->setText(QString("Email"));
        kyc_email_edit = new QLineEdit;
        kyc_email_edit->setText(QString::fromStdString(kycinfo.email));
        personalKYClayout->addWidget(kyc_email_label);
        personalKYClayout->addWidget(kyc_email_edit);
        QLabel* kyc_address_label = new QLabel();
        kyc_address_label->setText(QString("Address"));
        kyc_address_edit = new QLineEdit;
        kyc_address_edit->setText(QString::fromStdString(kycinfo.address));
        ...
        personalKYC->setLayout(personalKYClayout);
        ...
        if (settings.value("fLightTheme").toBool()) {
            QFile qFile(QString(":/css/light"));
            if (qFile.open(QFile::ReadOnly)) {
                QString styleSheet;
                styleSheet = QLatin1String(qFile.readAll());
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Anker Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::createTrayIconMenu()
{
    ...
    QAction* copyAnkerAddress = new QAction(QIcon(":/icons/edit"), tr("Anker Wallet"), this);
    copyAnkerAddress->setStatusTip(tr("Copy Anker Wallet address"));
    connect(copyAnkerAddress, SIGNAL(triggered()), this, SLOT(copyAnkerWallet()));

    QMenu* address = trayIconMenu->addMenu(tr("&Copy wallet address"));
    address->addAction(copyAnkerAddress);
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Anker network", "", count));
}
...
{
    QString strTitle = tr("Anker Core"); // default title
...
void BitcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    for (auto invoiceitem = invoices.begin(); invoiceitem != invoices.end();)
    {
        if (GUIUtil::parseBitcoinURI(QString::fromStdString(*invoiceitem), &rcp)) {
            QMessageBox::StandardButton retval = QMessageBox::question(this, QString("New invoice!"),
                QString("You have new invoice! Do you want to view?"),
                QMessageBox::Yes | QMessageBox::Cancel,
                QMessageBox::Cancel);
            if (retval == QMessageBox::Yes) emit receivedURI(QString::fromStdString(*invoiceitem));
        }
        invoiceitem = invoices.erase(invoiceitem);
    }
