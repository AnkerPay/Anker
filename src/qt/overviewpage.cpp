...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ANK)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sANKPercentage, QString& szANKPercentage)
{
    int nPrecision = 2;
    ...
    szANKPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sANKPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in anker.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in anker.conf");
    }
    ...
    bool showANKAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyANKAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showANKPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyANKPending = watchUnconfBalance != 0;
    bool showANKLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyANKLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showANKAvailable || showWatchOnlyANKAvailable);
    ui->labelBalanceText->setVisible(showANKAvailable || showWatchOnlyANKAvailable);
    ui->labelWatchAvailable->setVisible(showANKAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showANKPending || showWatchOnlyANKPending);
    ui->labelPendingText->setVisible(showANKPending || showWatchOnlyANKPending);
    ui->labelWatchPending->setVisible(showANKPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showANKLocked || showWatchOnlyANKLocked);
    ui->labelLockedBalanceText->setVisible(showANKLocked || showWatchOnlyANKLocked);
    ui->labelWatchLocked->setVisible(showANKLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ANK")
    updateDisplayUnit();
}
