...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ANK);
    ...
    if (!settings.contains("nAnonymizeAnkerAmount"))
        settings.setValue("nAnonymizeAnkerAmount", 1000);

    nAnonymizeAnkerAmount = settings.value("nAnonymizeAnkerAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAnkerAmount"))
        SoftSetArg("-anonymizeankeramount", settings.value("nAnonymizeAnkerAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in anker.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case Email:
            return QString::fromStdString(kycinfo.email);
        case Fio:
            return QString::fromStdString(kycinfo.fio);
        case Address:
            return QString::fromStdString(kycinfo.address);
        ...
        case AnonymizeAnkerAmount:
            return QVariant(nAnonymizeAnkerAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case Language:
            ...
            break;
        case Email:
            if (kycinfo.email != value.toString().toStdString()) {
                kycinfo.email = value.toString().toStdString();
                writetodb = true;
            }
            break;
        case Fio:
            if (kycinfo.fio != value.toString().toStdString()) {
                kycinfo.fio = value.toString().toStdString();
                writetodb = true;
            }
            break;
        case Address:
            if (kycinfo.address != value.toString().toStdString()) {
                kycinfo.address = value.toString().toStdString();
                writetodb = true;
            }
            break;
        case ZeromintEnable:
        ...
        case AnonymizeAnkerAmount:
            nAnonymizeAnkerAmount = value.toInt();
            settings.setValue("nAnonymizeAnkerAmount", nAnonymizeAnkerAmount);
            emit anonymizeAnkerAmountChanged(nAnonymizeAnkerAmount);
            break;
        case CoinControlFeatures:
