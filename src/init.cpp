...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "anker.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ankerd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ANK and zANK money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12365, 12366));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ANK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ANK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, anker, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ANK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Anker specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:12365"));
...
/*
    ...
    strUsage += HelpMessageOpt("-backupzank=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAnk minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zankbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zANK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
*/
#endif
//    strUsage += "  -anonymizeankeramount=<n>     " + strprintf(_("Keep N ANK anonymized (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Anker Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Anker is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Anker Core is shutting down."));
...
#endif
    // Make sure only a single Anker process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Anker Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (!coldstakConfig.read(strErr)) {
            string msg = strprintf("Error reading cold staking configuration file: %s\n", strErr.c_str());
            return InitError(msg);
        }
    }  // (!fDisableWallet)
#endif // ENABLE_WALLET
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Anker specific: zerocoin and spork DB's
                zerocoinDB = new CZerocoinDB(0, false, false);
                pSporkDB = new CSporkDB(0, false, false);
                pAliasDB = new CAliasesDB(0, false, false);
                ...
                // Anker: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZANKMinted();
                        RecalculateZANKSpent();
                    }
                    RecalculateANKSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Anker Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Anker Core to complete") << "\n";
        ...
        zwalletMain = new CzANKWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zANK wallet..."));

        bool fEnableZAnkBackups = GetBoolArg("-backupzank", true);
        pwalletMain->setZAnkAutoBackups(fEnableZAnkBackups);
    ...
    if ( (coldstakConfig.getCount() > -1) && pwalletMain) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Cold Staking coin:\n");
        uint256 mnTxHash;
        BOOST_FOREACH (CColdStakConfig::CColdStakEntry clds, coldstakConfig.getEntries()) {
            LogPrintf("  %s %s\n", clds.getTxHash(), clds.getOutputIndex());
            mnTxHash.SetHex(clds.getTxHash());
            COutPoint outpoint = COutPoint(mnTxHash, boost::lexical_cast<unsigned int>(clds.getOutputIndex()));
...
//    }
//
//    nAnonymizeAnkerAmount = GetArg("-anonymizeankeramount", 0);
//    if (nAnonymizeAnkerAmount > 999999) nAnonymizeAnkerAmount = 999999;
//    if (nAnonymizeAnkerAmount < 2) nAnonymizeAnkerAmount = 2;
    ...
    LogPrintf("Anonymize Anker Amount %d\n", nAnonymizeAnkerAmount);
    ...
    /* Denominations
       ...
       For example:
       1ANK+1000 == (.1ANK+100)*10
       10ANK+10000 == (1ANK+1000)*10
    */
