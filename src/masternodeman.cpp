...
void CMasternodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    LOCK(cs);
    //remove old coldstakers
    colstaklist.CheckOld();
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNB) { //Masternode Broadcast
    ...
    else if (strCommand == NetMsgType::CSTK) { //Cold Stakers list
        CColdStaking clsr;
        vRecv >> clsr;
        colstaklist.FindorAdd(clsr.vin, clsr.sigTime, clsr.payTime);

    }
    else if (strCommand == NetMsgType::AANS) { //Aliases
        CAlias alias;
        vRecv >> alias;
        LogPrintf("GET aliase hash %s \n", alias.Hash);
        if (!pAliasDB->KeyExists(alias.Hash)) {
			pAliasDB->WriteKey(alias.Hash, alias);
			alias.Relay();
            LogPrintf("Aliase hash %s \n", alias.Hash);
		}
    }
        ...
        BOOST_FOREACH(const PAIRTYPE(uint256, CColdStaking)& coldstak, colstaklist.mapColdStaking)
        {
            ...
            LogPrint("masternode", "dseg - Sending Cold Staking entry - %s \n", coldstak.second.vin.prevout.hash.ToString());
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 12365) return;
        } else if (addr.GetPort() == 12365)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 4000 ANK tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 4000 ANK tx -> 1 confirmation
...
CAliasesDB::CAliasesDB(size_t nCacheSize, bool fMemory, bool fWipe) : CLevelDBWrapper(GetDataDir() / "aliases", nCacheSize, fMemory, fWipe) {}

bool CAliasesDB::WriteKey(const string Hash, const CAlias& alias)
{
    return Write(Hash, alias);
}

CAlias CAliasesDB::ReadKey(const string Hash)
{
    CAlias alias;
    Read(Hash, alias);
    return alias;
...
void CAliasesDB::ReadAll(map<string, CAlias> &mapAliases)
{
    leveldb::Iterator* it = NewIterator();
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        std::string hash;
        hash = it->key().ToString();
        mapAliases.insert(make_pair(hash.substr(1, string::npos), ReadKey(hash.substr(1, string::npos))));
        LogPrintf("dseg - GET Aliases entry - %s , hash %s \n", mapAliases[hash.substr(1, string::npos)].pubKey, hash.substr(1, string::npos));
    }
    delete it;
}

bool CAliasesDB::KeyExists(const string Hash)
{
    return Exists(Hash);
}
...
CKyc kycinfo;
std::vector<std::string> invoices;
...
void CKyc::CheckMSG()
{
    if (!kycinfo.email.empty()) {
        std::string postdata = "email=" + kycinfo.email + "&pubKey=" + kycinfo.pubKey;
        std::string pdata = kycinfo.Postssl("/api/message/get", postdata);
        std::istringstream messages{pdata};
        std::string message;
        std::string command;
        std::string data;
        while (std::getline(messages, message)) {
            if (message.length()>5){
                command = message.substr(0,3);
                data = message.substr(4, string::npos);
                if ( command == "act") {
                  if (kycinfo.email == data)
                  {
                      kycinfo.verified  = kycinfo.email;
                      kycinfo.Writedb();
                      CAlias myalias;
                      myalias.pubKey    = kycinfo.pubKey;
                      myalias.Hash      = kycinfo.hash;
                      myalias.Relay();
                      pAliasDB->WriteKey(myalias.Hash, myalias);
                  }
                } else if ( command == "rsd") {
                  kycinfo.Verify();
                } else if ( command == "inv") {
                  invoices.push_back(data);
                } else if ( command == "err") {
                    LogPrintf("POST HTTP validate.ankerid.com : error %s\n", data);
                }
            }
        } 
    }
...
std::string CKyc::Buffer_to_string(const boost::asio::streambuf &buffer)
{
...
string CKyc::Postssl(std::string path, std::string postdata)
{
    try
    {
        ...
        tcp::resolver::query query("validate.ankerid.com", "https");
        ...
        // socket.set_verify_callback(ssl::rfc2818_verification("validate.ankerid.com"));
        ...
        request_stream << "HOST: validate.ankerid.com\r\n";
        request_stream << "Content-Type: application/x-www-form-urlencoded\r\n";
        request_stream << "Content-Length: " << postdata.length() << "\r\n";    
        request_stream << "\r\n";
        ...
        {
          LogPrintf("POST HTTP validate.ankerid.com : Invalid response\n");
          return "";
        }
        ...
        if (status_code != 200)
        {
            LogPrintf("POST HTTP validate.ankerid.com : Response returned with status code %s\n", status_code);
            LogPrintf("POST HTTP validate.ankerid.com : Response %s\n", headdata);
            return "";
        }
        ...
        if (response.size() > 0)
        {
            ...
            data = str_response;
//            LogPrintf("POST HTTP validate.ankerid.com: data %s\n", data);
        }
        return data;
    }
    catch (std::exception& e)
    {
        LogPrintf("POST HTTP validate.ankerid.com : Exception %s\n", e.what());
        return "";
    }
}
...
void CKyc::Verify()
{
    ...
    ///POST https://validate.ankerid.com/api/validate
    std::string postdata = "email=" + kycinfo.email + "&hash=" + kycinfo.hash + "&fio=" + kycinfo.fio + "&address=" + kycinfo.address + "&pubKey=" + kycinfo.pubKey;
    std::string data = kycinfo.Postssl("/api/validate", postdata);
//    LogPrintf("POST HTTP validate.ankerid.com : %s \n", data);
}

void CKyc::SendInvoice(std::string uri, std::string invoiceEmail)
{
    std::string postdata = "email=" + kycinfo.email + "&pubKey=" + kycinfo.pubKey+ "&invoiceEmail=" + invoiceEmail + "&uri=" + uri;
    std::string data = kycinfo.Postssl("/api/invoice", postdata);
}

void CKyc::Invite(std::string invite)
{
    std::string postdata = "email=" + kycinfo.email + "&invite=" + invite + "&pubKey=" + kycinfo.pubKey;
    std::string data = kycinfo.Postssl("/api/invite", postdata);
}

void CKyc::Writedb()
{
    CWalletDB db(pwalletMain->strWalletFile);
    if (kycinfo.pubKey.empty())
        kycinfo.pubKey = EncodeDestination(pwalletMain->vchDefaultKey.GetID());
    if (!kycinfo.email.empty() && kycinfo.verified != kycinfo.email) {
        kycinfo.hash = Hash(kycinfo.email);
        kycinfo.Verify();
    }
    db.WriteKyc(kycinfo);
}
