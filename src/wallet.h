...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ANK at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zANK send
enum ZerocoinSpendStatus {
    ZANK_SPEND_OKAY = 0,                            // No error
    ZANK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZANK_WALLET_LOCKED = 2,                         // Wallet was locked
    ZANK_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZANK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZANK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZANK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZANK_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZANK_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZANK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZANK_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZANK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZANK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZANK_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZANK_SPENT_USED_ZANK = 14,                      // Coin has already been spend
    ZANK_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZANK_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZAnkBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZANKOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzankAuto) const;
    ...
    */
    ...
    CzANKWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzANKTracker> zankTracker;
    ...
    void setZWallet(CzANKWallet* zwallet)
    {
        zwalletMain = zwallet;
        zankTracker = std::unique_ptr<CzANKTracker>(new CzANKTracker(strWalletFile));
    }

    CzANKWallet* getZWallet() { return zwalletMain; }
    ...
    void setZAnkAutoBackups(bool fEnabled)
    {
