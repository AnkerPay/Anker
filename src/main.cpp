...
// Copyright (c) 2017-2018 The Anker developers
...
#if defined(NDEBUG)
#error "Anker cannot be compiled without assertions."
#endif
...
CAliasesDB* pAliasDB = NULL;
map<uint256, string> mapAliasesHash;
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zANK is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zANK spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zANK without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zANK spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zANK spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zANK spend tx %s already in block %d",
            ...
            // Check that zANK mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetFullBlockValue(int nHeight)
{
    if (nHeight == 0) {
        ...
        return 5.31 * COIN;
    } else if (nHeight > 1451200 && nHeight <= 2502400) {
        return 4.52 * COIN;
    } else if (nHeight > 2502400 && nHeight <= 3553600) {
        return 3.84 * COIN;
    } else if (nHeight > 3553600 && nHeight <= 4604800) {
        return 3.26 * COIN;
    } else if (nHeight > 4604800 && nHeight <= 5656000) {
        return 2.77 * COIN;
    } else if (nHeight > 5656000 && nHeight <= 6707200) {
        return 2.36 * COIN;
    } else if (nHeight > 6707200 && nHeight <= 7758400) {
        return 2 * COIN;
    } else if (nHeight > 7758400 && nHeight <= 8809600) {
        return 1.7 * COIN;
    } else if (nHeight > 8809600 && nHeight <= 9860800) {
        return 1.45 * COIN;
    } else if (nHeight > 9860800 && nHeight <= 10912000) {
        return 1.23 * COIN;
    } else if (nHeight > 10912000 && nHeight <= 11963200) {
        return 1.05 * COIN;
    } else if (nHeight > 11963200 && nHeight <= 13014400) {
        return 0.89 * COIN;
    } else if (nHeight > 13014400 && nHeight <= 14065600) {
        return 0.76 * COIN;
    } else if (nHeight > 14065600 && nHeight <= 15116800) {
        return 0.64 * COIN;
    } else if (nHeight > 15116800 && nHeight <= 16168000) {
        return 0.55 * COIN;
    } else if (nHeight > 16168000 && nHeight <= 17219200) {
        return 0.46 * COIN;
    } else if (nHeight > 17219200 && nHeight <= 18270400) {
        return 0.39 * COIN;
    } else if (nHeight > 18270400 && nHeight <= 19321600) {
        return 0.34 * COIN;
    } else if (nHeight > 19321600 && nHeight <= 20372800) {
        return 0.28 * COIN;
    } else if (nHeight > 20372800 && nHeight <= 21424000) {
        return 0.24 * COIN;
    } else if (nHeight > 21424000 && nHeight <= 22475200) {
        return 0.21 * COIN;
    } else {
        return 0.18 * COIN;
    }
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 9860800) {
        fStakingPercentage = 0.15;
    }
    CAmount FullBlockValue = GetFullBlockValue(nHeight);
    CAmount BlockValue = FullBlockValue;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    int nMnActiveCount = (int) nMasternodeActiveHeight / threeMonthHeight;
    if (nMnActiveCount > 5) {
        nMnActiveCount = 5;
    }
    CAmount FullBlockValue = GetFullBlockValue(nHeight);
...
int64_t GetColdstakingPayment(int nHeight)
{
    double fSubsidyAdjustmentPercentage = 0.05;
    CAmount ret = GetFullBlockValue(nHeight) * fSubsidyAdjustmentPercentage;
    return ret;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Anker
...
{
    RenameThread("anker-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZANKMinted()
{
...
void RecalculateZANKSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zANK supply
...
bool RecalculateANKSupply(int nHeightStart)
{
...
{
    // Anker: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZANKSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zANKSpent: %s\n",
    ...
    //PoW and PoS phase redistributed fees to miner or staker.
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight);

    //Check that the block does not overmint
    if (!IsBlockValueValid(block, nExpectedMint, pindex->nMint)) { 
        return state.DoS(100, error("ConnectBlock() : reward pays too much (actual=%s vs limit=%s) nFees: %s",
                FormatMoney(pindex->nMint), FormatMoney(nExpectedMint), FormatMoney(nFees)), REJECT_INVALID, "bad-cb-amount");
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ANK to zANK
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zANK spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zANK serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zANK mints and %d zANK spends\n", __func__, nMints, nSpends);
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_MASTERNODE_ANNOUNCE:
        ...
        return false;
    case MSG_COLDSTAKER_ANNOUNCE:
        if (colstaklist.mapColdStaking.count(inv.hash)) {
            return true;
        }
        return false;
    case MSG_ALIASE_ANNOUNCE:
        if (mapAliasesHash.count(inv.hash)) {
            return true;
        }
        return false;
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (!pushed && inv.type == MSG_COLDSTAKER_ANNOUNCE) {
                    if (colstaklist.mapColdStaking.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << colstaklist.mapColdStaking[inv.hash];
                        pfrom->PushMessage(NetMsgType::CSTK, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_ALIASE_ANNOUNCE) {
                    if (mapAliasesHash.count(inv.hash)) {
                        if (pAliasDB->KeyExists(mapAliasesHash[inv.hash])) {
                            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                            ss.reserve(1000);
                            ss << pAliasDB->ReadKey(mapAliasesHash[inv.hash]);
                            pfrom->PushMessage(NetMsgType::AANS, ss);
                            pushed = true;
                        }
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Anker Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Anker Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Anker Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Anker: We use certain sporks during IBD, so check to see if they are
    ...
    // getaddr message mitigates the attack.
        ...
        BOOST_FOREACH (const CAddress& addr, vAddr)
            pfrom->PushAddress(addr);
        // send cold staking list
        BOOST_FOREACH(const PAIRTYPE(uint256, CColdStaking)& coldstak, colstaklist.mapColdStaking)
        {
            uint256 hash = colstaklist.mapColdStaking[coldstak.first].GetHash();
            LogPrint("net", "Sending Cold Staking entry - %s \n", coldstak.second.vin.prevout.hash.ToString());
            pfrom->PushInventory(CInv(MSG_COLDSTAKER_ANNOUNCE, hash));
        }
        // send aliases list
        map<string, CAlias> mapAliases;
        pAliasDB->ReadAll(mapAliases);
        BOOST_FOREACH(const PAIRTYPE(string, CAlias)& alias, mapAliases)
        {
            mapAliasesHash.insert(make_pair(mapAliases[alias.first].GetHash(), mapAliases[alias.first].Hash));
            pfrom->PushInventory(CInv(MSG_ALIASE_ANNOUNCE, mapAliases[alias.first].GetHash()));
            LogPrintf("dseg - Sending Aliases entry - %s \n", mapAliases[alias.first].pubKey);
        }
    }
