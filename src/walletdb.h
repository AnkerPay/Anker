...
public:
    ...
    bool WriteKyc(const CKyc& kyc);
    bool ReadKyc(CKyc& kyc);
    bool WriteZANKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZANKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZANKSeed_deprecated(uint256& seed);
    bool EraseZANKSeed();
    bool EraseZANKSeed_deprecated();

    bool WriteZANKCount(const uint32_t& nCount);
    bool ReadZANKCount(uint32_t& nCount);
