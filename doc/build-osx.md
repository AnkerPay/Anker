...
====================================
This guide will show you how to build ankerd (headless client) for OSX.
...
4.  (Optional) You can also install ankerd to your path:
...
4. Enter "anker-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `ankerd` for your own use.

ankerd/anker-cli binaries are not included in the anker-Qt.app bundle.

If you are building `ankerd` or `anker-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Anker-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./ankerd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./ankerd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=ankerrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Anker/anker.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Anker/anker.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Anker/debug.log

Other commands:
-------

    ./ankerd -daemon # to start the anker daemon.
    ./anker-cli --help  # for a list of command-line options.
    ./anker-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Anker with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Anker wallet:
...
You have the choice to build the GUI Anker wallet as a Mac OSX app, described in “How to build the Anker-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Anker-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Anker-Qt.dmg inside your Anker folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/Anker/src/qt/anker-qt